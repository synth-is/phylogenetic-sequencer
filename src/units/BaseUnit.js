...
export class BaseUnit {
  ...
  async getAudioData(soundData, renderParams, options = {}) {
    ...
    try {
      ...
      // Skip VFS operations if renderer is not available
      if (!renderer) {
        console.warn(`${this.type}Unit: Renderer not available, attempting audio fetch without VFS`);
      }
      ...
      // Update VFS with the audio data only if renderer is available
      if (audioBuffer && renderer) {
        try {
          const vfsUpdate = {};
          vfsUpdate[vfsKey] = audioBuffer.getChannelData(0);
          await renderer.updateVirtualFileSystem(vfsUpdate);
        } catch (vfsError) {
          console.warn(`${this.type}Unit: Failed to update VFS, continuing without it:`, vfsError);
        }
      }
